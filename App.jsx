...
const css = {
  ...
  topbar: { background:G.card, borderBottom:`1px solid ${G.cardBorder}`, padding:"0 24px", display:"flex", alignItems:"center", gap:16, height:60, position:"sticky", top:0, zIndex:100 },
  logo: { fontSize:20, fontWeight:700, color:G.accent, letterSpacing:"-0.5px" },
  nav: { display:"flex", gap:4, marginLeft:"auto", flexWrap:"wrap" },
  navBtn: (a) => ({ padding:"6px 14px", borderRadius:8, border:"none", background:a?G.accent:"transparent", color:a?"#fff":G.muted, fontWeight:500, fontSize:13, cursor:"pointer" }),
  main: { flex:1, padding:"28px 24px", maxWidth:1100, margin:"0 auto", width:"100%", boxSizing:"border-box" },
  card: { background:G.card, border:`1px solid ${G.cardBorder}`, borderRadius:14, padding:22 },
  h1: { fontSize:22, fontWeight:700, margin:"0 0 20px", letterSpacing:"-0.3px" },
...
function Modal({ title, onClose, children, wide }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"#000a", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ ...css.card, width:"100%", maxWidth:wide?700:520, maxHeight:"90vh", overflowY:"auto", position:"relative" }}>
        <div style={{ display:"flex", alignItems:"center", marginBottom:20 }}>
          <span style={{ fontWeight:700, fontSize:16 }}>{title}</span>
          <button onClick={onClose} style={{ marginLeft:"auto", background:"none", border:"none", color:G.muted, fontSize:24, cursor:"pointer" }}>×</button>
        </div>
        {children}
      </div>
    </div>
...
function Spinner() {
  return <div style={{ textAlign:"center", padding:40, color:G.muted }}>⏳ Carregando...</div>;
}
...
function useData() {
  ...
  return { caregivers, setCaregivers, shifts, setShifts, extras, setExtras, loading, error, reload: loadAll };
}

// ─── Dashboard ────────────────────────────────────────────────────────────────
function Dashboard({ caregivers, shifts, extras }) {
  const td = today();
  const [wStart, wEnd] = getWeekRange(td);
  const mStr = td.slice(0, 7);
  ...
  const todayAll  = all.filter(x => x.date === td);
  const weekAll   = all.filter(x => x.date >= wStart && x.date <= wEnd);
  const monthAll  = all.filter(x => x.date.startsWith(mStr));
  const pendAll   = all.filter(x => x.payment_status === "Pending");

  const pendByCg = caregivers
    .map(c => ({ c, val: sum(pendAll.filter(x => x.caregiver_id === c.id)) }))
    .filter(x => x.val > 0);

  const todayShifts = shifts.filter(s => s.shift_date === td);
  const todayExtras = extras.filter(e => e.charge_date === td);

  return (
    <div>
      <h1 style={css.h1}>📊 Dashboard</h1>
      <div style={{ ...css.grid(2), marginBottom:20 }}>
        <StatCard label="Gasto Hoje"            value={fmt(sum(todayAll))}  color={G.green}  icon="📅" />
        <StatCard label="Gasto na Semana"       value={fmt(sum(weekAll))}   color={G.accent} icon="📆" />
        <StatCard label="Gasto no Mês"          value={fmt(sum(monthAll))}  color={G.purple} icon="🗓️" />
        <StatCard label="Pendente de Pagamento" value={fmt(sum(pendAll))}   color={G.amber}  icon="⏳" />
      </div>
        ...
        <div style={{ ...css.card, marginBottom:20 }}>
          ...
          {pendByCg.map(({ c, val }) => (
            <div key={c.id} style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:`1px solid ${G.cardBorder}22` }}>
              <span>{c.name}</span>
              <span style={{ color:G.amber, fontWeight:700 }}>{fmt(val)}</span>
            </div>
          ))}
        </div>
      )}

      {(todayShifts.length > 0 || todayExtras.length > 0) && (
        <div style={css.card}>
          ...
          {todayShifts.map(s => {
            ...
            return (
              <div key={s.id} style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:`1px solid ${G.cardBorder}22` }}>
                <div>
                  <span style={{ marginRight:8 }}>{shiftIcon[s.shift_type]}</span>
                  <span>{cg?.name || "—"}</span>
                  <span style={{ ...css.badge(shiftColor[s.shift_type]), marginLeft:8 }}>{shiftLabel[s.shift_type]}</span>
                </div>
...
function Caregivers({ caregivers, setCaregivers }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        const updated = await api.put(`/caregivers/${editing}`, body);
        setCaregivers(caregivers.map(c => c.id === editing ? updated : c));
      } else {
        const created = await api.post("/caregivers", body);
        setCaregivers([...caregivers, created]);
      }
      close();
    } catch { setError("Erro ao salvar. Tente novamente."); } finally { setSaving(false); }
  };

  const toggle = async (cg) => {
    const updated = await api.put(`/caregivers/${cg.id}`, { ...cg, active: !cg.active });
    setCaregivers(caregivers.map(c => c.id === cg.id ? updated : c));
  };
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", marginBottom:20 }}>
        ...
        <button style={{ ...css.btn(), marginLeft:"auto" }} onClick={() => open()}>+ Nova Cuidadora</button>
      </div>
      <div style={css.card}>
...
function ExtraChargeModal({ caregivers, onClose, onSaved, initial }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      let result;
      if (initial?.id) result = await api.put(`/extra-charges/${initial.id}`, body);
      else result = await api.post("/extra-charges", body);
      onSaved(result);
    } catch { setError("Erro ao salvar."); } finally { setSaving(false); }
  };

  return (
    <Modal title={initial?.id ? "Editar Cobrança Avulsa" : "Nova Cobrança Avulsa"} onClose={onClose}>
      <ErrorBanner msg={error} />
      ...
      <div style={css.grid(2)}>
        <Field label="Data">
          <input type="date" style={css.input} value={form.charge_date} onChange={e=>setForm({...form,charge_date:e.target.value})} />
        </Field>
      ...
      <Field label="Descrição *">
        <input style={css.input} value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Ex: Aplicação de insulina, Curativo, Fisioterapia..." />
      </Field>
      <div style={css.grid(2)}>
        <Field label="Valor (R$) *">
          <input type="number" style={css.input} value={form.value} onChange={e=>setForm({...form,value:e.target.value})} placeholder="0,00" />
        </Field>
        <Field label="Status">
...
function Shifts({ caregivers, shifts, setShifts, extras, setExtras }) {
  const [tab, setTab] = useState("shifts"); // "shifts" | "extras"
  ...
  const [formS, setFormS] = useState(blankS);
  ...
  const getVal = (cgId, type) => {
    const cg = caregivers.find(c=>c.id===cgId);
    if (!cg) return "";
    return type==="Day12h"?cg.day_shift_value:type==="Night12h"?cg.night_shift_value:cg.full_day_shift_value;
  };

  const openS = (s=null) => { setEditingS(s?.id||null); setFormS(s?{...s, caregiver_id:s.caregiver_id}:blankS); setShowModalS(true); };
  ...
  const saveShift = async () => {
    ...
    if (editingS) {
      const updated = await api.put(`/shifts/${editingS}`, body);
      setShifts(shifts.map(s=>s.id===editingS?updated:s));
    } else {
  ...
  const toggleShiftPay = async (s) => {
    const st = s.payment_status==="Paid"?"Pending":"Paid";
    const updated = await api.patch(`/shifts/${s.id}/payment`, { payment_status:st, payment_date:st==="Paid"?today():null });
    setShifts(shifts.map(x=>x.id===s.id?updated:x));
  };

  const delExtra = async (id) => { if (!confirm("Remover?")) return; await api.delete(`/extra-charges/${id}`); setExtras(extras.filter(e=>e.id!==id)); };
  const toggleExtraPay = async (e) => {
    const st = e.payment_status==="Paid"?"Pending":"Paid";
    const updated = await api.patch(`/extra-charges/${e.id}/payment`, { payment_status:st, payment_date:st==="Paid"?today():null });
    setExtras(extras.map(x=>x.id===e.id?updated:x));
  };
  ...
  const totalS = filteredS.reduce((a,s)=>a+Number(s.value),0);
  const totalE = filteredE.reduce((a,e)=>a+Number(e.value),0);

  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", marginBottom:20, gap:12, flexWrap:"wrap" }}>
        <h1 style={{ ...css.h1, margin:0 }}>📋 Lançamentos</h1>
        <input type="month" style={{ ...css.input, width:160 }} value={filter.month} onChange={e=>setFilter({...filter,month:e.target.value})} />
        <select style={{ ...css.select, width:200 }} value={filter.caregiverId} onChange={e=>setFilter({...filter,caregiverId:e.target.value})}>
          <option value="">Todas as cuidadoras</option>
          {caregivers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          <button style={css.btn()} onClick={() => openS()}>+ Plantão</button>
          <button style={css.btn(G.teal)} onClick={() => { setEditingExtra(null); setShowModalE(true); }}>+ Avulso 💊</button>
        </div>
      </div>

      {/* Tab switcher */}
      <div style={{ display:"flex", gap:0, marginBottom:16, background:G.card, borderRadius:10, border:`1px solid ${G.cardBorder}`, overflow:"hidden", width:"fit-content" }}>
        {[["shifts",`🌿 Plantões (${filteredS.length})`],["extras",`💊 Avulsos (${filteredE.length})`]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{ padding:"9px 20px", border:"none", background:tab===id?G.accent:"transparent", color:tab===id?"#fff":G.muted, fontWeight:600, fontSize:13, cursor:"pointer" }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ ...css.card, marginBottom:16, padding:"14px 20px" }}>
        <div style={{ display:"flex", gap:24, flexWrap:"wrap" }}>
          <span style={{ color:G.muted, fontSize:13 }}>Plantões: <strong style={{ color:G.text }}>{fmt(totalS)}</strong></span>
          <span style={{ color:G.muted, fontSize:13 }}>Avulsos: <strong style={{ color:G.teal }}>{fmt(totalE)}</strong></span>
          <span style={{ color:G.muted, fontSize:13 }}>Total mês: <strong style={{ color:G.green }}>{fmt(totalS+totalE)}</strong></span>
          <span style={{ color:G.muted, fontSize:13 }}>Pendente: <strong style={{ color:G.amber }}>{fmt([...filteredS,...filteredE].filter(x=>x.payment_status==="Pending").reduce((a,x)=>a+Number(x.value),0))}</strong></span>
        </div>
      </div>
              ...
              {filteredS.map(s=>{
                ...
                return (
                  <tr key={s.id} style={{ opacity:s.payment_status==="Paid"?0.6:1 }}>
                    <td style={css.td}>{fmtDate(s.shift_date)}</td>
              ...
              {filteredE.map(e=>{
                ...
                return (
                  <tr key={e.id} style={{ opacity:e.payment_status==="Paid"?0.6:1 }}>
                    <td style={css.td}>{fmtDate(e.charge_date)}</td>
                    <td style={css.td}>{cg?.name||"—"}</td>
        ...
        <Modal title={editingS?"Editar Plantão":"Novo Plantão"} onClose={closeS}>
          ...
          <Field label="Tipo de Plantão">
            <div style={{ display:"flex", gap:8 }}>
              {[["Day12h","☀️ Diurno 12h"],["Night12h","🌙 Noturno 12h"],["Full24h","🌗 24 horas"]].map(([v,l])=>(
      ...
      {showModalE && (
        <ExtraChargeModal
          ...
          onSaved={(result) => {
            if (editingExtra?.id) setExtras(extras.map(e=>e.id===result.id?result:e));
            else setExtras([...extras, result]);
            setShowModalE(false); setEditingExtra(null);
          }}
        />
      )}
    </div>
...
function Calendar({ caregivers, shifts, extras }) {
  ...
  const byDay = useMemo(() => {
    const map = {};
    shifts.filter(s=>s.shift_date.startsWith(prefix)).forEach(s=>{
      const d=parseInt(s.shift_date.split("-")[2]);
      if(!map[d]) map[d]={shifts:[],extras:[]};
      map[d].shifts.push(s);
    });
    extras.filter(e=>e.charge_date.startsWith(prefix)).forEach(e=>{
      const d=parseInt(e.charge_date.split("-")[2]);
      if(!map[d]) map[d]={shifts:[],extras:[]};
      map[d].extras.push(e);
    });
    return map;
    ...
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ fontWeight:700, minWidth:140, textAlign:"center" }}>{monthName(month)} {year}</span>
          <button style={css.btnGhost} onClick={()=>changeMonth(1)}>›</button>
        </div>
      </div>

      <div style={{ display:"flex", gap:20, alignItems:"flex-start", flexWrap:"wrap" }}>
        <div style={{ ...css.card, flex:1, minWidth:300, padding:16 }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3, marginBottom:6 }}>
            {["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"].map(d=><div key={d} style={{ textAlign:"center", fontSize:10, fontWeight:700, color:G.muted, padding:"3px 0" }}>{d}</div>)}
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3 }}>
            {Array(firstDay).fill(null).map((_,i)=><div key={"e"+i} />)}
            {Array(days).fill(null).map((_,i)=>{
              const d=i+1;
              const data=byDay[d]||{shifts:[],extras:[]};
              const total=[...data.shifts,...data.extras].reduce((a,x)=>a+Number(x.value),0);
              const isToday=`${year}-${String(month).padStart(2,"0")}-${String(d).padStart(2,"0")}`===today();
              const isSel=selected===d;
              return (
                <div key={d} onClick={()=>setSelected(d===selected?null:d)} style={{ background:isSel?G.accent:isToday?G.accent+"22":G.input, border:`1px solid ${isSel?G.accent:isToday?G.accent:G.cardBorder}`, borderRadius:7, padding:"5px 6px", minHeight:70, cursor:"pointer" }}>
                  <div style={{ fontSize:11, fontWeight:700, color:isSel?"#fff":isToday?G.accent:G.muted, marginBottom:2 }}>{d}</div>
                  {data.shifts.slice(0,2).map(s=>{
                    const cg=caregivers.find(c=>c.id===s.caregiver_id);
                    return <div key={s.id} style={{ fontSize:9, padding:"1px 3px", borderRadius:3, marginBottom:1, background:isSel?"#ffffff22":shiftColor[s.shift_type]+"22", color:isSel?"#fff":shiftColor[s.shift_type], overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis" }}>{shiftIcon[s.shift_type]} {cg?.name?.split(" ")[0]||"?"}</div>
                  })}
                  {data.extras.slice(0,1).map(e=>(
                    <div key={e.id} style={{ fontSize:9, padding:"1px 3px", borderRadius:3, marginBottom:1, background:isSel?"#ffffff22":G.teal+"22", color:isSel?"#fff":G.teal, overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis" }}>💊 {e.description}</div>
                  ))}
                  {total>0&&<div style={{ fontSize:9, fontWeight:700, color:isSel?"#fff":G.green, marginTop:1 }}>{fmt(total)}</div>}
                </div>
              );
            })}
          </div>
        ...
        {/* Day detail panel */}
        {selected && (
          <div style={{ ...css.card, width:300, flexShrink:0 }}>
            <h2 style={{ ...css.h2, marginBottom:16 }}>📅 {fmtDate(selectedDate)}</h2>
            {selData?.shifts.length===0 && selData?.extras.length===0 && <div style={{ color:G.muted, fontSize:13 }}>Nenhum lançamento</div>}
            {selData?.shifts.map(s=>{
              const cg=caregivers.find(c=>c.id===s.caregiver_id);
              return (
            ...
            {selData?.extras.map(e=>{
              const cg=caregivers.find(c=>c.id===e.caregiver_id);
              return (
                <div key={e.id} style={{ padding:"10px 0", borderBottom:`1px solid ${G.cardBorder}22` }}>
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <span style={{ fontWeight:600 }}>{cg?.name||"—"}</span>
                    <span style={{ fontWeight:700, color:G.teal }}>{fmt(e.value)}</span>
                  </div>
                  <div style={{ display:"flex", gap:6, marginTop:4 }}>
...
function WeeklyClose({ caregivers, shifts, setShifts, extras, setExtras }) {
  ...
  const [wStart, wEnd] = getWeekRange(weekDate);
  ...
  const markAllPaid = async (cgId) => {
    const toPayS = weekShifts.filter(s=>s.caregiver_id===cgId&&s.payment_status==="Pending");
    const toPayE = weekExtras.filter(e=>e.caregiver_id===cgId&&e.payment_status==="Pending");
    const td = today();
    const updS = await Promise.all(toPayS.map(s=>api.patch(`/shifts/${s.id}/payment`,{payment_status:"Paid",payment_date:td})));
    const updE = await Promise.all(toPayE.map(e=>api.patch(`/extra-charges/${e.id}/payment`,{payment_status:"Paid",payment_date:td})));
    ...
    <div>
      ...
      <div style={{ ...css.card, marginBottom:20 }}>
        <div style={{ display:"flex", gap:16, alignItems:"flex-end", flexWrap:"wrap" }}>
          <div>
            <label style={css.label}>Semana de referência</label>
            <input type="date" style={{ ...css.input, width:200 }} value={weekDate} onChange={e=>setWeekDate(e.target.value)} />
          </div>
          <div style={{ padding:"10px 16px", background:G.accent+"22", borderRadius:8, color:G.accent, fontWeight:600, fontSize:13 }}>
      ...
      {weeklyCgs.map(cg=>{
        const cgS=weekShifts.filter(s=>s.caregiver_id===cg.id);
        const cgE=weekExtras.filter(e=>e.caregiver_id===cg.id);
        if(cgS.length===0&&cgE.length===0) return null;
        const totalS=sum(cgS), totalE=sum(cgE), total=totalS+totalE;
        const pend=sum([...cgS,...cgE].filter(x=>x.payment_status==="Pending"));
        const allPaid=[...cgS,...cgE].every(x=>x.payment_status==="Paid");

        return (
          <div key={cg.id} style={{ ...css.card, marginBottom:16 }}>
            <div style={{ display:"flex", alignItems:"center", marginBottom:12, flexWrap:"wrap", gap:8 }}>
              ...
              {!allPaid&&pend>0&&<button style={{ ...css.btn(G.green), marginLeft:"auto", padding:"8px 16px" }} onClick={()=>markAllPaid(cg.id)}>Marcar como Pago ({fmt(pend)})</button>}
            </div>

            {cgS.length>0&&<>
              <div style={{ fontSize:12, fontWeight:700, color:G.muted, marginBottom:6, textTransform:"uppercase" }}>Plantões</div>
              <table style={{ ...css.table, marginBottom:12 }}>
                <thead><tr>{["Data","Tipo","Valor","Status"].map(h=><th key={h} style={css.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {cgS.map(s=>(
                    <tr key={s.id} style={{ opacity:s.payment_status==="Paid"?0.6:1 }}>
                      <td style={css.td}>{fmtDate(s.shift_date)}</td>
            ...
            {cgE.length>0&&<>
              <div style={{ fontSize:12, fontWeight:700, color:G.teal, marginBottom:6, textTransform:"uppercase" }}>💊 Avulsos</div>
              <table style={{ ...css.table, marginBottom:12 }}>
                <thead><tr>{["Data","Descrição","Valor","Status"].map(h=><th key={h} style={css.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {cgE.map(e=>(
                    <tr key={e.id} style={{ opacity:e.payment_status==="Paid"?0.6:1 }}>
                      <td style={css.td}>{fmtDate(e.charge_date)}</td>
                      <td style={css.td}>{e.description}</td>
                      <td style={{ ...css.td, fontWeight:700, color:G.teal }}>{fmt(e.value)}</td>
                      <td style={css.td}><span style={css.badge(e.payment_status==="Paid"?G.green:G.amber)}>{e.payment_status==="Paid"?"✓ Pago":"Pendente"}</span></td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div style={{ display:"flex", gap:24, paddingTop:8, borderTop:`1px solid ${G.cardBorder}` }}>
              {cgS.length>0&&<span style={{ fontSize:13, color:G.muted }}>Plantões: <strong style={{ color:G.text }}>{fmt(totalS)}</strong></span>}
              {cgE.length>0&&<span style={{ fontSize:13, color:G.muted }}>Avulsos: <strong style={{ color:G.teal }}>{fmt(totalE)}</strong></span>}
      ...
      {(weekShifts.length>0||weekExtras.length>0)&&(
        <div style={{ ...css.card, background:G.accent+"11", border:`1px solid ${G.accent}44` }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              <div style={{ fontWeight:700 }}>Total Geral da Semana</div>
              <div style={{ fontSize:12, color:G.muted, marginTop:2 }}>Plantões: {fmt(sum(weekShifts))} · Avulsos: {fmt(sum(weekExtras))}</div>
            </div>
            <span style={{ fontWeight:800, fontSize:22, color:G.accent }}>{fmt(sum(weekShifts)+sum(weekExtras))}</span>
          </div>
        </div>
      )}
    </div>
...
function MonthlyClose({ caregivers, shifts, setShifts, extras, setExtras }) {
  ...
  const markAllPaid = async (cgId) => {
    const td = today();
    const toPayS = monthShifts.filter(s=>s.caregiver_id===cgId&&s.payment_status==="Pending");
    const toPayE = monthExtras.filter(e=>e.caregiver_id===cgId&&e.payment_status==="Pending");
    const updS = await Promise.all(toPayS.map(s=>api.patch(`/shifts/${s.id}/payment`,{payment_status:"Paid",payment_date:td})));
    const updE = await Promise.all(toPayE.map(e=>api.patch(`/extra-charges/${e.id}/payment`,{payment_status:"Paid",payment_date:td})));
    setShifts(shifts.map(s=>{ const u=updS.find(x=>x.id===s.id); return u||s; }));
    setExtras(extras.map(e=>{ const u=updE.find(x=>x.id===e.id); return u||e; }));
  ...
  const totalAll = sum(monthShifts)+sum(monthExtras);
  const paidAll  = sum([...monthShifts,...monthExtras].filter(x=>x.payment_status==="Paid"));
  const pendAll  = sum([...monthShifts,...monthExtras].filter(x=>x.payment_status==="Pending"));

  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        <h1 style={{ ...css.h1, margin:0 }}>🗓️ Fechamento Mensal</h1>
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:8 }}>
          <button style={css.btnGhost} onClick={()=>changeMonth(-1)}>‹</button>
          <span style={{ fontWeight:700, minWidth:160, textAlign:"center" }}>{monthName(month)} {year}</span>
          <button style={css.btnGhost} onClick={()=>changeMonth(1)}>›</button>
        </div>
      </div>

      <div style={{ ...css.grid(3), marginBottom:20 }}>
        <StatCard label="Total do Mês" value={fmt(totalAll)} color={G.accent} icon="💰" />
        <StatCard label="Total Pago"   value={fmt(paidAll)}  color={G.green}  icon="✅" />
        <StatCard label="Pendente"     value={fmt(pendAll)}  color={G.amber}  icon="⏳" />
      </div>

      {allCgs.length===0&&<div style={{ ...css.card, color:G.muted, textAlign:"center", padding:40 }}>Nenhum lançamento neste mês</div>}

      {allCgs.map(cg=>{
        const cgS=monthShifts.filter(s=>s.caregiver_id===cg.id);
        const cgE=monthExtras.filter(e=>e.caregiver_id===cg.id);
        const day   = sum(cgS.filter(s=>s.shift_type==="Day12h"));
        const night = sum(cgS.filter(s=>s.shift_type==="Night12h"));
        const full  = sum(cgS.filter(s=>s.shift_type==="Full24h"));
        const totalS=sum(cgS), totalE=sum(cgE), total=totalS+totalE;
        const pend=sum([...cgS,...cgE].filter(x=>x.payment_status==="Pending"));
        const allPaid=[...cgS,...cgE].every(x=>x.payment_status==="Paid");
        ...
        return (
          <div key={cg.id} style={{ ...css.card, marginBottom:16 }}>
            <div style={{ display:"flex", alignItems:"center", marginBottom:14, flexWrap:"wrap", gap:8 }}>
              ...
              {allPaid&&<span style={css.badge(G.green)}>✓ Pago</span>}
              {!allPaid&&pend>0&&<button style={{ ...css.btn(G.green), marginLeft:"auto", padding:"8px 16px" }} onClick={()=>markAllPaid(cg.id)}>Marcar como Pago</button>}
            </div>

            <div style={{ display:"flex", gap:20, flexWrap:"wrap", marginBottom:12 }}>
              {day>0&&<div><div style={{ fontSize:11, color:G.amber, fontWeight:600 }}>☀️ Diurno</div><div style={{ fontWeight:700 }}>{fmt(day)}</div></div>}
              {night>0&&<div><div style={{ fontSize:11, color:G.purple, fontWeight:600 }}>🌙 Noturno</div><div style={{ fontWeight:700 }}>{fmt(night)}</div></div>}
              {full>0&&<div><div style={{ fontSize:11, color:G.green, fontWeight:600 }}>🌗 24h</div><div style={{ fontWeight:700 }}>{fmt(full)}</div></div>}
              {totalE>0&&<div><div style={{ fontSize:11, color:G.teal, fontWeight:600 }}>💊 Avulsos</div><div style={{ fontWeight:700, color:G.teal }}>{fmt(totalE)}</div><div style={{ fontSize:11, color:G.muted }}>{cgE.length} item(ns)</div></div>}
              <div style={{ marginLeft:"auto" }}>
                <div style={{ fontSize:11, color:G.muted, fontWeight:600 }}>TOTAL</div>
                <div style={{ fontWeight:800, fontSize:20, color:G.accent }}>{fmt(total)}</div>
              </div>
            </div>
...
// ─── App ─────────────────────────────────────────────────────────────────────
const TABS = [["dash","🏠 Início"],["shifts","📋 Lançamentos"],["cal","📅 Calendário"],["weekly","📆 Semanal"],["monthly","🗓️ Mensal"],["cgs","👩‍⚕️ Cuidadoras"]];

export default function App() {
  const [tab, setTab] = useState("dash");
  const { caregivers, setCaregivers, shifts, setShifts, extras, setExtras, loading, error } = useData();

  if (loading) return <div style={{ ...css.page, alignItems:"center", justifyContent:"center" }}><Spinner /></div>;
  ...
  return (
    <div style={css.page}>
      <div style={css.topbar}>
        ...
        <nav style={css.nav}>
          {TABS.map(([id,label])=><button key={id} style={css.navBtn(tab===id)} onClick={()=>setTab(id)}>{label}</button>)}
        </nav>
      </div>
      <main style={css.main}>
